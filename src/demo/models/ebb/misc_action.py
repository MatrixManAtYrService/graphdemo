...
class MiscAction(BaseModel, MinimalMiscActionMixin):
    """
    Pydantic V2 model for the `misc_action` table.
    """
    id: int = Field(..., description="Primary key, auto-incrementing.", gt=0)
    uuid: str = Field(..., max_length=26, description="Unique identifier for the misc action.")
    ...
    misc_action_fee_code_uuid: Optional[str] = Field(default=None, max_length=26, description="UUID for the misc action fee code.")
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                ...
                "misc_action_fee_code_uuid": "01H8X7Y7Z7QWERTYUIOPASDFGA",
...
if __name__ == "__main__":
    example_dict = MiscAction.model_json_schema().get("example", {})
    misc_action_instance = MiscAction(**example_dict)
    print("----begin example: misc-action----")
    print(misc_action_instance.model_dump_json(indent=2))
    print("----begin minmal example: misc-action----")
    print(json.dumps(misc_action_instance.to_minimal_dict(), indent=2))
    print("----end: misc-action----")