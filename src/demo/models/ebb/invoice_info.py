...
class InvoiceInfo(BaseModel, MinimalInvoiceInfoMixin):
    """
    Pydantic V2 model for the `invoice_info` table.
    """
    id: int = Field(..., gt=0)
    ...
    invoice_num: str = Field(..., max_length=30)
...
if __name__ == "__main__":
    example_dict = InvoiceInfo.model_json_schema().get("example", {})
    invoice_info_instance = InvoiceInfo(**example_dict)
    print("----begin example: invoice-info----")
    print(invoice_info_instance.model_dump_json(indent=2))
    print("----begin minmal example: invoice-info----")
    print(json.dumps(invoice_info_instance.to_minimal_dict(), indent=2))
