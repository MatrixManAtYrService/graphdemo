...
class MinimalCellularActionErrorMixin:
...
class CellularActionError(BaseModel, MinimalCellularActionErrorMixin):
    """
    Pydantic V2 model for the `cellular_action_error` table.
    """
    id: int = Field(..., description="Primary key, auto-incrementing.", gt=0)
    uuid: str = Field(..., max_length=26, description="Unique identifier for the cellular action error.")
    cellular_action_uuid: str = Field(..., max_length=26, description="UUID of the related cellular action.")
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                ...
                "cellular_action_uuid": "01H8X7Y7Z7QWERTYUIOPASDFGZ",
                ...
                "error_details": "Unable to validate cellular usage with carrier API due to timeout",
                "resolved": 0,
...
if __name__ == "__main__":
    example_dict = CellularActionError.model_json_schema().get("example", {})
    cellular_action_error_instance = CellularActionError(**example_dict)
    print("----begin example: cellular-action-error----")
    print(cellular_action_error_instance.model_dump_json(indent=2))
    print("----begin minmal example: cellular-action-error----")
    print(json.dumps(cellular_action_error_instance.to_minimal_dict(), indent=2))

    print("----end: cellular-action-error----")