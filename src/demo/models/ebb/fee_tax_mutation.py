...
class MinimalFeeTaxMutationMixin:
...
class FeeTaxMutation(BaseModel, MinimalFeeTaxMutationMixin):
    """
    Pydantic V2 model for the `fee_tax_mutation` table.
    """
    id: int = Field(..., gt=0)
    mutation_action: MutationAction
    mutation_timestamp: datetime.datetime
    fee_tax_id: int = Field(..., gt=0)
...
if __name__ == "__main__":
    example_dict = FeeTaxMutation.model_json_schema().get("example", {})
    fee_tax_mutation_instance = FeeTaxMutation(**example_dict)
    print("----begin example: fee-tax-mutation----")
    print(fee_tax_mutation_instance.model_dump_json(indent=2))
    print("----begin minmal example: fee-tax-mutation----")
    print(json.dumps(fee_tax_mutation_instance.to_minimal_dict(), indent=2))

    print("----end: fee-tax-mutation----")