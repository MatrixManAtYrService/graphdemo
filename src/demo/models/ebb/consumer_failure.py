...
class MinimalConsumerFailureMixin:
...
class ConsumerFailure(BaseModel, MinimalConsumerFailureMixin):
    """
    Pydantic V2 model for the `consumer_failure` table.
    """
    id: int = Field(..., description="Primary key, auto-incrementing.", gt=0)
    uuid: str = Field(..., max_length=36, description="Unique identifier for the consumer failure.")
...
if __name__ == "__main__":
    example_dict = ConsumerFailure.model_json_schema().get("example", {})
    consumer_failure_instance = ConsumerFailure(**example_dict)
    print("----begin example: consumer-failure----")
    print(consumer_failure_instance.model_dump_json(indent=2))
    print("----begin minmal example: consumer-failure----")
    print(json.dumps(consumer_failure_instance.to_minimal_dict(), indent=2))

    print("----end: consumer-failure----")