...
class MinimalAdjustActionFeeCodeMixin:
...
class AdjustActionFeeCode(BaseModel, MinimalAdjustActionFeeCodeMixin):
    """
    Pydantic V2 model for the `adjust_action_fee_code` table.
    """
    id: int = Field(..., description="Primary key, auto-incrementing.", gt=0)
    uuid: str = Field(..., max_length=26, description="Unique identifier for the adjust action fee code.")
...
if __name__ == "__main__":
    example_dict = AdjustActionFeeCode.model_json_schema().get("example", {})
    adjust_action_fee_code_instance = AdjustActionFeeCode(**example_dict)
    print("----begin example: adjust-action-fee-code----")
    print(adjust_action_fee_code_instance.model_dump_json(indent=2))
    print("----begin minmal example: adjust-action-fee-code----")
    print(json.dumps(adjust_action_fee_code_instance.to_minimal_dict(), indent=2))

    print("----end: adjust-action-fee-code----")
