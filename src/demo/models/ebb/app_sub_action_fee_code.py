...
class MinimalAppSubActionFeeCodeMixin:
...
class AppSubActionFeeCode(BaseModel, MinimalAppSubActionFeeCodeMixin):
    """
    Pydantic V2 model for the `app_sub_action_fee_code` table.
    """
    id: int = Field(..., description="Primary key, auto-incrementing.", gt=0)
    uuid: str = Field(..., max_length=26, description="Unique identifier for the app sub action fee code.")
...
if __name__ == "__main__":
    example_dict = AppSubActionFeeCode.model_json_schema().get("example", {})
    app_sub_action_fee_code_instance = AppSubActionFeeCode(**example_dict)
    print("----begin example: app-sub-action-fee-code----")
    print(app_sub_action_fee_code_instance.model_dump_json(indent=2))
    print("----begin minmal example: app-sub-action-fee-code----")
    print(json.dumps(app_sub_action_fee_code_instance.to_minimal_dict(), indent=2))

    print("----end: app-sub-action-fee-code----")