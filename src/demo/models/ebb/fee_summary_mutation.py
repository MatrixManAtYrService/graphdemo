...
class MinimalFeeSummaryMutationMixin:
...
class FeeSummaryMutation(BaseModel, MinimalFeeSummaryMutationMixin):
    """
    Pydantic V2 model for the `fee_summary_mutation` table.
    """
    id: int = Field(..., gt=0)
    mutation_action: MutationAction
    mutation_timestamp: datetime.datetime
    fee_summary_id: int = Field(..., gt=0)
    ...
    exclude_from_invoice: Optional[int] = Field(default=None)
...
if __name__ == "__main__":
    example_dict = FeeSummaryMutation.model_json_schema().get("example", {})
    fee_summary_mutation_instance = FeeSummaryMutation(**example_dict)
    print("----begin example: fee-summary-mutation----")
    print(fee_summary_mutation_instance.model_dump_json(indent=2))
    print("----begin minmal example: fee-summary-mutation----")
    print(json.dumps(fee_summary_mutation_instance.to_minimal_dict(), indent=2))

    print("----end: fee-summary-mutation----")